...
namespace tamp {
    ...
    namespace detail {
        ...
        /**
         * @brief \c const \p T if \p P is const qualified, \p T otherwise
         *
         * @tparam T
         * @tparam P
         */
        template<typename T, typename P>
        using same_const_t = std::conditional_t< std::is_const_v<P>, const T, T>;

        template<typename T, typename P>
        static inline same_const_t<T,P>* p(P* const ptr) noexcept {
            return static_cast<same_const_t<T,P>*>(static_cast<same_const_t<void,P>*>(ptr));
        }


        template<int32_t INC, typename T>
        static inline void __attribute__((always_inline)) incptr(T*& ptr) noexcept {
            ptr = static_cast<T*>(p<same_const_t<uint8_t,T>>(ptr) + INC);
        }

        template<typename T, typename P>
        static inline same_const_t<T,P> __attribute__((always_inline)) as(P* const ptr) noexcept {
            return *p<T>(ptr);
        }
         ...
         */
        template<uint32_t M>
        static constexpr uint32_t multof(const uint32_t x) noexcept {
            static_assert(M!=0);
            if constexpr (M==1) {
                return x;
            } else
         ...
         * @return uint32_t
         */
        static inline uint32_t cmp8(const void* d1, const void* d2, const uint32_t len) noexcept {
                    ...
                    if constexpr (arch::XTENSA && arch::XT_LOOP) {
                        ...
                        asm (
                            "LOOPNEZ %[cnt], .Lend_%=" "\n"
                                ...
                                "BEQ %[tmp_16], %[v], .L_found_0_%=" "\n" // -> found at data-3

                                "EXTUI %[tmp_16], %[tmp_32], 8, 16" "\n" // extract bits 23...8
                                "BEQ %[tmp_16], %[v], .L_found_1_%=" "\n" // -> found at data-2

                                "EXTUI %[tmp_16], %[tmp_32], 16, 16" "\n" // extract bits 31...16
                                "BEQ %[tmp_16], %[v], .L_found_2_%=" "\n" // -> found at data-1
                    ...
                    if constexpr (arch::XTENSA && arch::XT_LOOP) {
                        ...
                        asm (
                            "LOOPNEZ %[cnt], .Lend_%=" "\n"
                                ...
                                "BEQ %[tmp_24], %[vl], .L_found_lo_%=" "\n"

                                "SRLI %[tmp_24], %[tmp_32], 8" "\n"
                                "BEQ %[tmp_24], %[vh], .L_found_hi_%=" "\n"
        ...
        template<>
        struct FindImpl<arch::Arch_t::ESP32S3> {
    ...
    class Locator {
                    ...
                    do {
                        ...
                        if(match) {
                            ...
                            if(searchLen < maxSearchLen) [[likely]] {
                                ...
                                // we'll take that too.
                                matchLen += detail::cmp8(pattern+matchLen, match+matchLen, maxSearchLen-matchLen);
                            }
        ...
        private:
            ...
            static const uint8_t* __attribute__((noinline)) find_pattern(const uint8_t* const pattern, const uint32_t patLen, const uint8_t* const data, const uint32_t dataLen) noexcept {
                ...
                return detail::FindImpl<arch::Arch>::find_pattern(pattern, patLen, data, dataLen);
            }
